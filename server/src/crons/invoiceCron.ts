            ...
            for (const invoice of transitioned) {
                ...
                try {
                    ...
                    if (clientInfoRes.rows.length > 0) {
                        ...
                        await client.query(
                            "UPDATE invoices SET payment_url = $1, payment_link_id = $2 WHERE id = $3",
                            [paymentLinkResponse.short_url, paymentLinkResponse.id, invoice.id]
                        );
                    }
                } catch (linkErr: any) {
                ...
                if (reminderType) {
                    ...
                    if (invoice.reminder_email_enabled && invoice.client_email) {
                        try {
                            ...
                            emailSuccess = true;
                            await client.query(
                                "INSERT INTO invoice_reminders_log (invoice_id, medium, status) VALUES ($1, $2, 'Success')",
                                [invoice.invoice_id, "Email"]
                            );
                        } catch (e) {
                            await client.query(
                                "INSERT INTO invoice_reminders_log (invoice_id, medium, status) VALUES ($1, $2, 'Failed')",
                                [invoice.invoice_id, "Email"]
                            );
                        }
                    }
