...
export const InvoiceServices = {
        ...
        try {
            ...
            if (!resolvedClientId && clientName) {
                ...
                const checkClient = await client.query(
                    "SELECT id FROM clients WHERE user_id = $1 AND (email = $2 OR name = $3)",
                ...
                if (checkClient.rows.length > 0) {
                    ...
                    const newClientRes = await client.query(
                        "INSERT INTO clients (user_id, name, email, phone, address) VALUES ($1, $2, $3, $4, $5) RETURNING id",
                        [userId, clientName, clientEmail || null, clientPhone || null, clientAddress || null]
                    );
            ...
            const invoiceResult = await client.query(
                ...
                [userId, resolvedClientId, invoiceNumber, dueDate, taxAmount, totalAmount, notes]
            );
            ...
            for (const item of items) {
                ...
                if (!resolvedItemId && item.name) {
                    ...
                    if (checkItem.rows.length > 0) {
                        ...
                        const newItemRes = await client.query(
                            "INSERT INTO items (user_id, name, default_price) VALUES ($1, $2, $3) RETURNING id",
                ...
                await client.query(
                    ...
                    VALUES ($1, $2, $3, $4)`,
                    [newInvoice.id, resolvedItemId, item.quantity, item.price]
                );
            }
    ...
    async getInvoiceDetails(invoiceId: string, userId: string) {
        ...
        const itemsRes = await pool.query(
            `SELECT ii.*, it.name, it.description 
    ...
    async updateInvoiceStatus(invoiceId: string, userId: string, status: string) {
    ...
    async updateInvoicePdfUrl(invoiceId: string, userId: string, pdfUrl: string) {
        const result = await pool.query(
            "UPDATE invoices SET pdf_url = $1 WHERE id = $2 AND user_id = $3 RETURNING *",
            [pdfUrl, invoiceId, userId]
        );
        return result.rows[0] || null;
    },

    async updateInvoicePaymentLink(invoiceId: string, userId: string, paymentUrl: string, paymentLinkId: string) {
        const result = await pool.query(
            "UPDATE invoices SET payment_url = $1, payment_link_id = $2, status = 'Sent' WHERE id = $3 AND user_id = $4 RETURNING *",
            [paymentUrl, paymentLinkId, invoiceId, userId]
        );
        return result.rows[0] || null;
    }
};
