...
export const AuthServices = {
    ...
    async getUserById(userId: string) {
        const result = await pool.query(
            "SELECT id, name, email, created_at, subscription_plan, razorpay_account_id, reminder_whatsapp_enabled, reminder_email_enabled, business_name, business_address, phone, bank_account_number, bank_ifsc FROM users WHERE id = $1", 
            [userId]
        );
    ...
    async resetPasswordWithToken(token: string, passwordString: string) {
