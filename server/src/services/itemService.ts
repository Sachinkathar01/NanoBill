...
export const ItemServices = {
    async createItem(userId: string, name: string, description: string, defaultPrice: number, imageUrl: string | null = null) {
        const newItem = await pool.query(
            "INSERT INTO items (user_id, name, description, default_price, image_url) VALUES ($1, $2, $3, $4, $5) RETURNING *",
            [userId, name, description, defaultPrice, imageUrl]
        );
    ...
    async updateItem(itemId: string, userId: string, name: string, description: string, defaultPrice: number, imageUrl: string | null = null) {
        let query = "UPDATE items SET name = $1, description = $2, default_price = $3";
        const values: any[] = [name, description, defaultPrice];
