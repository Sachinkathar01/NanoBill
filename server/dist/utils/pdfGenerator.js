...
export const generatePDFBuffer = async (templateData) => {
    ...
    return await new Promise((resolve, reject) => {
        ...
        if (user.business_address)
            sellerInfoText += `${user.business_address}\n`;
        sellerInfoText += `Email: ${user.email || 'billing@nanobill.com'}`;
        if (user.phone)
            sellerInfoText += `  |  Phone: ${user.phone}`;
        ...
        if (invoice.client_address)
            clientDetails += `${invoice.client_address}\n`;
        if (invoice.client_email)
            clientDetails += `${invoice.client_email}\n`;
        if (invoice.client_phone)
            clientDetails += `Phone: ${invoice.client_phone}`;
        ...
        if (qrBuffer) {
            ...
            if (user.bank_account_number)
                bankText += `A/C Number: ${user.bank_account_number}\n`;
            if (user.bank_ifsc)
                bankText += `IFSC Code: ${user.bank_ifsc}\n`;
